...
export function LoadChapterContentPlugin(): null {
    ...
    useEffect(() => {
        if (!hasLoaded && currentChapter?.content && currentChapter.id === currentChapterId) {
            // Defer setEditorState out of the React commit phase to avoid the
            // "flushSync called from inside a lifecycle method" warning — Lexical
            // calls flushSync internally when applying a new editor state.
            queueMicrotask(() => {
                try {
